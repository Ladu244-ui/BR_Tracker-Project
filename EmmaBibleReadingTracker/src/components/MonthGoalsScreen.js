...
export default function MonthGoalsScreen({ onBack, viewingDate }) {
  const dateToUse = viewingDate || new Date();
  const storageKey = `month_goals_${dateToUse.getFullYear()}_${dateToUse.getMonth()}`;
  const monthName = dateToUse.toLocaleDateString('en-ZA', { month: 'long', year: 'numeric' });
  ...
  useEffect(() => { load(); }, [viewingDate]);

  async function load() {
    const raw = await AsyncStorage.getItem(storageKey);
    if (raw) setGoals(JSON.parse(raw));
    const n = await AsyncStorage.getItem(`month_notes_${dateToUse.getFullYear()}_${dateToUse.getMonth()}`);
    if (n) setNotes(n);
  }
  ...
  async function saveNotes(text) {
    setNotes(text);
    await AsyncStorage.setItem(`month_notes_${dateToUse.getFullYear()}_${dateToUse.getMonth()}`, text);
  }
