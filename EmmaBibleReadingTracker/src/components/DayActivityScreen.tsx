...
const STATUS_OPTIONS = ['Done', 'In Progress', 'Not Done', 'Skipped', '—'] as const;
...
export default function DayActivityScreen({ dayIndex, date, weekNum, onBack }: Props) {
  const dateKey = date.toISOString().split('T')[0];
  const dayName = DAY_NAMES[dayIndex];

  const [activities, setActivities] = useState<Activity[]>([EMPTY_ACTIVITY()]);
  ...
  async function saveEvaluation(text: string) {
    setEvaluation(text);
